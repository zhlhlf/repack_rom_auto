...
if [ "$(uname -m)" == "x86_64" ] && [  "$(uname)" == "Linux" ];then
    ...
    apt update -y
    apt upgrade -y
    apt install -y brotli cpio aria2 python3 busybox zip unzip p7zip-full openjdk-8-jre zipalign zstd bc android-sdk-libsparse-utils xmlstarlet
    if [ $? -ne 0 ];then
        echo "安装可能出错，请手动执行：apt brotli cpio install -y aria2 python3 busybox zip unzip p7zip-full openjdk-8-jre zipalign zstd bc xmlstarlet"
    fi
fi
